...
MainWindow::MainWindow(QWidget *parent)
  ...
  connect(ui->pushButton_color_edges, &QPushButton::clicked, this,
          &MainWindow::openColorDialogEdges);
  connect(ui->pushButton_color_vertices, &QPushButton::clicked, this,
          &MainWindow::openColorDialogVertices);
  connect(ui->pushButton_color_back, &QPushButton::clicked, this,
          &MainWindow::openColorDialogBackground);
  connect(&colorDialog, &QColorDialog::colorSelected, this,
          &MainWindow::colorSelected);
  connect(this, SIGNAL(colorChanged(ColorSettings::CurrentParam, const QColor)),
          ui->view_window,
          SLOT(onColorChanged(ColorSettings::CurrentParam, const QColor)));

  connect(this, SIGNAL(projectionTypeChanged(int)), ui->view_window,
          SLOT(updateProjectionType(int)));
  connect(this, SIGNAL(widthEdgeChanged(int)), ui->view_window,
          SLOT(updateWidthEdge(int)));
  connect(this, SIGNAL(lineTypeChanged(int)), ui->view_window,
          SLOT(updateLineType(int)));
  connect(this, SIGNAL(sizeVerticesChanged(int)), ui->view_window,
          SLOT(updateSizeVertices(int)));
  connect(this, SIGNAL(verticesTypeChanged(int)), ui->view_window,
          SLOT(updateVerticesType(int)));
...
void MainWindow::loadSettings() {
  ...
  colorSettings.vertices_color =
      QColor(vertices_red, vertices_green, vertices_blue);
...
bool MainWindow::isValidAndNotEmptyFile() {
  return !ui->file_name->text().isEmpty() && vertices.size() - 1 != 0 &&
         amount_surfaces != 0 && ui->error_file->text().isEmpty();
}

bool MainWindow::isAngles(double degree_x, double degree_y, double degree_z) {
  return (fabs(degree_x) >= 0 && fabs(degree_x) <= 360 && fabs(degree_y) >= 0 &&
          fabs(degree_y) <= 360 && fabs(degree_z) >= 0 &&
          fabs(degree_z) <= 360);
}
...
void MainWindow::on_pushButton_save_image_clicked() {
  if (isValidAndNotEmptyFile()) {
    ...
    QString file_path = QFileDialog::getSaveFileName(
        this, nullptr, QString(), "JPEG (*.jpeg);; BMP (*.bmp)",
...
void MainWindow::on_pushButton_make_screencast_clicked() {
  if (isValidAndNotEmptyFile() &&
      ui->pushButton_make_screencast->text() == "Записать скринкаст") {
...
void MainWindow::makeScreencast() {
  ...
  QString screencast_path = QFileDialog::getSaveFileName(
      this, nullptr, QString(), "GIF Image Files (*.gif)", nullptr,
