...
void GlView::setupOpenGLState() {
    glClearColor(colorSettings.background_color.redF(), colorSettings.background_color.greenF(), colorSettings.background_color.blueF(), 1.0f);
...
void GlView::setupProjection() {
    ...
    if (renderingSettings.projection_type == RenderingSettings::ProjectionType::Parallel) {
      glOrtho(-1, 1, -1, 1, -1, 99999);
    } else if (renderingSettings.projection_type == RenderingSettings::ProjectionType::Central) {
...
void GlView::drawObjects() {
    ...
    for (int i = 0; i < count_surfaces; ++i) {
      ...
      for (int j = 0; j < 3; ++j) {
        glColor3d(colorSettings.edges_color.redF(), colorSettings.edges_color.greenF(), colorSettings.edges_color.blueF());
    ...
    if (renderingSettings.vertices_type != RenderingSettings::VerticesType::Default) {
      if (renderingSettings.vertices_type == RenderingSettings::VerticesType::Circular) {
      ...
      for (int i = 0; i < count_surfaces; ++i) {
        glBegin(GL_POINTS);
        for (int j = 0; j < 3; ++j) {
            glColor3d(colorSettings.vertices_color.redF(), colorSettings.vertices_color.greenF(), colorSettings.vertices_color.blueF());
            glVertex3d(all_vertices[all_surfaces[i][j]][0],
                   all_vertices[all_surfaces[i][j]][1],
                   all_vertices[all_surfaces[i][j]][2]);
        }
...
void GlView::updateProjectionType(int index) {
    switch (index) {
        case 0:
            renderingSettings.projection_type = RenderingSettings::ProjectionType::Central;
            break;
        case 1:
            renderingSettings.projection_type = RenderingSettings::ProjectionType::Parallel;
            break;
    }
...
void GlView::onColorChanged(ColorSettings::CurrentParam param, const QColor &color) {
    switch (param) {
